...
module Nginx
  ...
  def self.write_fullproxy_version_config(version_key, http_port, https_port,
    ...
    always_secure_locations = secure_handlers[:always].map { |handler|
      HelperFunctions.generate_secure_location_config(handler, https_port, concat_location_urls)
    }.join
    never_secure_locations = secure_handlers[:never].map { |handler|
      HelperFunctions.generate_secure_location_config(handler, http_port, concat_location_urls)
    }.join
